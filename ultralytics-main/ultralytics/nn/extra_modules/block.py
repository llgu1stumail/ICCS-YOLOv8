...
class MHSA_CGLU(nn.Module):
    def __init__(self,
        ...
        # self.attention = EMA(inc)
        # self.attention = SimAM(inc)
    def forward(self, x):
        shortcut = x
        x = self.drop_path(self.mhsa(self.norm1(x))) + shortcut
        # x = self.drop_path(self.attention(self.norm1(x))) + shortcut
...
class PartiallyTransformerBlock(nn.Module):
    def __init__(self, c, tcr, kernel_size=13, shortcut=True) -> None:
        super().__init__()
        self.t_ch = int(c * tcr)
        self.c_ch = c - self.t_ch
        # self.c_b = Bottleneck(self.c_ch, self.c_ch, shortcut=True)
        # self.c_b = EIEM(self.c_ch, self.c_ch)
        # self.c_b = Star_Block(self.c_ch)
        self.c_b = SMPCGLU(self.c_ch, kernel_size)
